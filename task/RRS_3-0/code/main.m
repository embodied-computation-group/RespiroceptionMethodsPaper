...
openSmallWin    = 1;        % open small window for debugging
...
if vars.RRSTdevice == 1
    if (exist('RD','var')==0) || isnan(RD.respDevice)
        ...
        % Calibrate the device. This takes about 1 minute
...
try
    ...
    %% Show welcome screen during calibration
    calibrationTime         = 65;        % seconds

    % Gray screen - Take a short break and press 'space' to
    % continue, also shows time remaining
    [~, startPause] = Screen('Flip', scr.win);
    
    
    while (GetSecs - startPause) <= calibrationTime
        % Update counter
        tRemaining = ceil((calibrationTime - (GetSecs - startPause)));
        counterText = strcat(vars.InstructionCalib, num2str(tRemaining), ' sec');
